...
def connect():
    ...
    return mdb.connect(
        host='localhost', 
        user='abadia2', 
        password='423510c156a3f2680ee62f3fff42ca1c05fa87b05551f18fa5130b5dfe1017cc', 
        db='project2'
    )
...
def createUser(username, password):
    ...
    statement = "INSERT INTO users (username, password, passwordhash) VALUES (%s, %s, %s);"
    cur.execute(statement, (username, password, password_hash)) 
    db_rw.commit()
...
def addHistory(user_id, query):
    ...
    statement = "INSERT INTO history (user_id, query) VALUES (%s, %s);"
    cur.execute(statement, (user_id, query))
    db_rw.commit()
...
def getHistory(user_id):
    ...
    statement = "SELECT query FROM history WHERE user_id=%s ORDER BY id DESC LIMIT 15;"
